...
type
  ...
  private
    FDataTypeCheckBoxes: array[dtFirstNumericDataType..dtLastNumericDataType] of
    TCheckbox;
...
const
  FONT_SIZES: array[0..14] of PtrInt =
...
begin
  ...
  pbOffice.Height := CommonData.Images.WidthForPPI[CommonData.Images.Width,
    Font.PixelsPerInch];
...
procedure TSettingsForm.PageControlChanging(Sender: TObject; var AllowChange: boolean);
begin
...
begin
  ...
  begin
    ...
    if cbHexPrefix.ItemIndex = 0 then
      ...
      OffsetDisplayHexPrefix :=
        copy(cbHexPrefix.Text, 1, pos(' ', cbHexPrefix.Text) - 1);
    ...
    DataViewerDataTypes := [];
    for dt := dtFirstNumericDataType to dtLastNumericDataType do
      if FDataTypeCheckBoxes[dt].Checked then
...
begin
  ...
  stream := TStringStream.Create(
    #$1B#$FF#$08#$00'Here is some sample text loaded into our demo HexEditor.'#$0D#$0A#09#$AE);
  try
...
begin
  ...
  begin
    ...
    for i := 0 to cbBytesPerRow.Items.Count - 1 do
    begin
    ...
    for i := 0 to cbBytesPerColumn.Items.Count - 1 do
    begin
...
function TSettingsForm.ValidData(out AControl: TWinControl; out AMsg: string): boolean;
begin
  Result := False;
